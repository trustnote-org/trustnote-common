		...
		function(cb){ // parent units
			...
			function checkForUnstablePredecessors(){
				conn.query(
					...
					UNION \n\
					SELECT unit FROM units JOIN address_definition_changes USING(unit) \n\
		...
		function(cb){ // input coins
			...
			if (bGenesis){
				if(objUnit.authors.length>1)
					 objPaymentMessage.payload.inputs = [{type: "issue", serial_number: 1, amount: constants.TOTAL_WHITEBYTES, address: params.witnesses[0]}];
				else
