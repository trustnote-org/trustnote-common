...
function trimParentList(conn, arrParentUnits, address, handleTrimmedList) {
	if (arrParentUnits.length <= constants.MAX_PARENTS_PER_UNIT)
		return findConflictingParentUnit(conn, address, arrParentUnits, handleTrimmedList);
	conn.query(
		...
		[constants.MAX_PARENTS_PER_UNIT],
		function (rows) {
			var parents = rows.map(function (row) {
				return row.unit;
			});
			return findConflictingParentUnit(conn, address, parents, handleTrimmedList);
		});
}


function findConflictingParentUnit(conn, address, parents, handleTrimmedList) {
	conn.query("select max(latest_included_mc_index) as maxParentLimci from units where unit in (?)", [parents], function (rows) {
		if (!rows[0].maxParentLimci) {
			rows[0].maxParentLimci = Number.MAX_VALUE;
		}
		conn.query("SELECT unit \n\
			FROM units JOIN unit_authors USING(unit) \n\
			WHERE address=? AND sequence='good' AND (main_chain_index>? OR main_chain_index IS NULL) \n\
			AND unit NOT IN (?) \n\
			ORDER BY level DESC LIMIT 1", [address, rows[0].maxParentLimci, parents], function (rowss) {
				if (rowss.length > 0) {
					graph.determineIfIncludedOrEqual(conn, rowss[0].unit, parents, function (bIncluded) {
						if (!bIncluded) {
							console.error("findConflictingParentUnit add a unit", rowss[0].unit);
							if (parents.length === constants.MAX_PARENTS_PER_UNIT)
								parents[constants.MAX_PARENTS_PER_UNIT - 1] = rowss[0].unit;
							else
								parents.push(rowss[0].unit);
						}
						handleTrimmedList(parents.sort());
					});
				} else
					handleTrimmedList(parents.sort());
			});
...
function pickParentUnitsAndLastBall(conn, address, onDone) {
		...
		findLastStableMcBall(conn, function (err, last_stable_mc_ball, last_stable_mc_ball_unit, last_stable_mc_ball_mci) {
			...
			adjustLastStableMcBallAndParents(
				...
				function (last_stable_ball, last_stable_unit, last_stable_mci, arrAdjustedParentUnits) {
					trimParentList(conn, arrAdjustedParentUnits, address, function (arrTrimmedParentUnits) {
